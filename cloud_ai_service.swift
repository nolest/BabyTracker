...
class CloudAIService {
    ...
    private func convertToPredictionResult(_ response: DeepseekPredictionResponse, babyId: String) -> PredictionResult {
        ...
        if let nextFeeding = response.nextFeeding {
            ...
            let earliestStartTime = calendar.date(
                byAdding: .minute,
                value: nextFeeding.earliestStartMinutes,
                to: today
            ) ?? now
            
            let latestStartTime = calendar.date(
                byAdding: .minute,
                value: nextFeeding.latestStartMinutes,
                to: today
            ) ?? now
            ...
            nextFeedingPrediction = NextFeedingPrediction(
                earliestStartTime: earliestStartTime,
                latestStartTime: latestStartTime,
                expectedDuration: TimeInterval(nextFeeding.expectedDurationMinutes * 60),
                confidence: nextFeeding.confidence
            )
        } else {
        ...
        if let nextActivity = response.nextActivity {
            let now = Date()
            let calendar = Calendar.current
            let today = calendar.startOfDay(for: now)
            ...
            let earliestStartTime = calendar.date(
                byAdding: .minute,
                value: nextActivity.earliestStartMinutes,
                to: today
            ) ?? now
            
            let latestStartTime = calendar.date(
                byAdding: .minute,
                value: nextActivity.latestStartMinutes,
                to: today
            ) ?? now
            
            // 轉換活動類型
            let activityType = ActivityType(rawValue: nextActivity.activityType) ?? .other
            
            nextActivityPrediction = NextActivityPrediction(
                activityType: activityType,
                earliestStartTime: earliestStartTime,
                latestStartTime: latestStartTime,
                confidence: nextActivity.confidence
            )
        } else {
        ...
        return PredictionResult(
            ...
            nextFeedingPrediction: nextFeedingPrediction,
            nextActivityPrediction: nextActivityPrediction,
            basedOnRecordsCount: 0, // 雲端分析不提供這個信息
            basedOnDaysCount: 14, // 假設使用過去14天的數據
            basedOnPatternType: "cloud_analysis",
            validUntil: Calendar.current.date(byAdding: .hour, value: 12, to: Date()) ?? Date(),
            isCloudAnalysis: true,
            recommendations: response.recommendations
        )
    }
...
enum CloudError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidAPIKey:
            return NSLocalizedString("無效的API密鑰。請在設置中檢查您的API密鑰。", comment: "")
        case .networkError:
            return NSLocalizedString("網絡錯誤。請檢查您的網絡連接。", comment: "")
        case .serverError:
            return NSLocalizedString("服務器錯誤。請稍後再試。", comment: "")
        case .insufficientData:
            return NSLocalizedString("數據不足，無法進行雲端分析。", comment: "")
        case .rateLimitExceeded:
            return NSLocalizedString("已超出API調用限制。請稍後再試。", comment: "")
        case .timeout:
            return NSLocalizedString("請求超時。請檢查您的網絡連接。", comment: "")
        case .unknownError:
            return NSLocalizedString("發生未知錯誤。請稍後再試。", comment: "")
        }
    }
}
...
// 擴展SleepPatternResult以支持雲端分析標識
extension SleepPatternResult {
    var isCloudAnalysis: Bool
    ...
    init(
        ...
        environmentFactors: SleepEnvironmentFactors,
        trend: SleepTrend,
        recommendations: [String],
        confidenceScore: Double,
        isCloudAnalysis: Bool
    ) {
        self.sleepPatternType = sleepPatternType
        ...
        self.isCloudAnalysis = isCloudAnalysis
    }
}

// 擴展RoutineAnalysisResult以支持雲端分析標識
extension RoutineAnalysisResult {
    var isCloudAnalysis: Bool
    
    init(
        regularityScore: Int,
        typicalPatterns: [RoutinePattern],
        ...
        self.regularityScore = regularityScore
        self.typicalPatterns = typicalPatterns
...
// 擴展PredictionResult以支持雲端分析標識和建議
extension PredictionResult {
    ...
    init(
        ...
        nextFeedingPrediction: NextFeedingPrediction?,
        nextActivityPrediction: NextActivityPrediction?,
        basedOnRecordsCount: Int,
        basedOnDaysCount: Int,
        basedOnPatternType: String,
        validUntil: Date,
        isCloudAnalysis: Bool = false,
        recommendations: [String]? = nil
    ) {
        self.babyId = babyId
        self.predictionTimestamp = predictionTimestamp
        self.confidenceScore = confidenceScore
        self.nextSleepPrediction = nextSleepPrediction
        self.nextFeedingPrediction = nextFeedingPrediction
        self.nextActivityPrediction = nextActivityPrediction
        self.basedOnRecordsCount = basedOnRecordsCount
        self.basedOnDaysCount = basedOnDaysCount
        self.basedOnPatternType = basedOnPatternType
...
struct ActivityDistributionData {
    let activityType: String
    let percentage: Double
    let averageDuration: Double
    let preferredTimeRanges: [PreferredTimeRange]
}

struct PreferredTimeRange {
    let startMinutes: Int
    let endMinutes: Int
    let frequency: Double
}

struct SleepEnvironmentFactors {
    let lightImpact: EnvironmentFactorImpact
    let noiseImpact: EnvironmentFactorImpact
    let temperatureImpact: EnvironmentFactorImpact
    let humidityImpact: EnvironmentFactorImpact
}

struct EnvironmentFactorImpact {
    let level: ImpactLevel
